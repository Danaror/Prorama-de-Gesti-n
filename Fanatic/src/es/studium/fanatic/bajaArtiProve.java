...
public class bajaArtiProve implements WindowListener, ActionListener, ItemListener {
	//atributos
		Frame ventana = new Frame ("Baja Artículo//Proveedor");
		Label lblElegir = new Label ("Elegir Línea a borrar");
		...
		private void rellenarChoice() 
		{
			...
			try {
				while (rs.next())
				{
					chElegirLinea.add(rs.getInt("idProveedorArticulo") + "-" +
		...
		public void actionPerformed(ActionEvent pulsarBoton) 
		{
			...
			}else  if (pulsarBoton.getSource().equals(btnSi))
			{
				...
				String sentencia = "DELETE FROM proveedoresarticulos WHERE idProveedorArticulo ="+array[0]+";";
